...
const DEFAULT_SESSION_KEY_PATH = './session.pem';
...
export const config = {
  ...
  publishManifestIntervalMs: parseInt(process.env.PUBLISH_MANIFEST_INTERVAL_MS || String(DEFAULT_PUBLISH_MANIFEST_INTERVAL_MS), 10),
  processIntervalMs: parseInt(process.env.PROCESS_INTERVAL_MS || String(DEFAULT_PROCESS_INTERVAL_MS), 10),
  debug: process.env.DEBUG === 'true',
  pubCert: process.env.PUB_CERT,
  privKey: process.env.PRIV_KEY,
  healthPort: parseInt(process.env.HEALTH_PORT || String(DEFAULT_HEALTH_PORT), 10),
  customC2paManifest: process.env.CUSTOM_C2PA_MANIFEST,
  cleanupMaxAgeMinutes: parseInt(process.env.CLEANUP_MAX_AGE_MINUTES || String(DEFAULT_CLEANUP_MAX_AGE_MINUTES), 10),
  cleanupIntervalMs: parseInt(process.env.CLEANUP_INTERVAL_MS || String(DEFAULT_CLEANUP_INTERVAL_MS), 10),
  useVsiMethod: process.env.USE_VSI_METHOD === 'true',
  streamId: process.env.STREAM_ID || 'live',
  sessionKeyPath: process.env.SESSION_KEY_PATH || DEFAULT_SESSION_KEY_PATH,
